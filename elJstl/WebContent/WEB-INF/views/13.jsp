...
<body>
	...
	<ul>
		<!-- begin:시작값, end:끝깞, step:간격, var:개별값들을담을변수명 -->
	...
	<table border=1>
		...
		<!-- c:forEach, items:Collection 객체(배열, List, Map), var:items로부터 추출된 개별 객체에 붙이는 변수명, varStatus:개별객체(var)가 실제컬렉션(items)에서 위치(index)와 카운트(몇번째 요소인가) -->
		<c:forEach items="${userList }" var="user" varStatus="status">
			<tr>
				<td>${user.no }</td>
				<td>${user.name }</td>
				<td>${user.email }</td>
				<!-- varStauts : 해당 객체의 컬렉션 내부에서 상태값 -->
				<td>${status.index }</td>
				<td>${status.count }</td>
			</tr>
		</c:forEach>
